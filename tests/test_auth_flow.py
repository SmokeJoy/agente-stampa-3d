...
def mock_settings(tmp_path: Path):
    ...
    # Ripristina le settings originali dopo il test
...
def test_load_credentials_new_token_flow(
    ...
    # Assicura che il file token non esista all'inizio del test
    if mock_token_file.exists():
        mock_token_file.unlink()
    
    # Crea un finto client_secret.json per evitare FileNotFoundError nel flusso
    mock_client_secret_file.write_text(
        '''{"installed": {"client_id": "test_client_id", "project_id": "test_project_id", 
           "auth_uri": "https://accounts.google.com/o/oauth2/auth", 
           "token_uri": "https://oauth2.googleapis.com/token", 
           "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs", 
           "client_secret": "test_client_secret", "redirect_uris": ["http://localhost"]}}'''
    )

    # Configura i mock per InstalledAppFlow
    mock_flow_instance = MockInstalledAppFlow.from_client_secrets_file.return_value
    mock_creds_object = MockCredentials.return_value # Oggetto credenziali mockato
    mock_creds_object.to_json.return_value = '''{"token": "mock_token_value"}''' # JSON mockato
    mock_creds_object.valid = True # Simula credenziali valide
...
@patch("services.google_calendar.auth_flow.Credentials")
@patch("services.google_calendar.auth_flow.InstalledAppFlow") # Anche se non usato direttamente, patch per coerenza
def test_load_credentials_existing_valid_token(
    MockInstalledAppFlow: MagicMock, # Non usato ma presente per la patch
    MockCredentials: MagicMock,
    mock_settings: tuple[Path, Path]
):
    """Testa il caricamento di credenziali valide da un token.json esistente."""
    mock_token_file, _ = mock_settings # mock_client_secret_file non è necessario qui

    # Crea un finto token.json valido
    mock_valid_token_json = '''{"token": "valid_mock_token", "refresh_token": "mock_refresh", "scopes": ["test_scope"], "client_id": "id", "client_secret": "secret"}'''
    mock_token_file.write_text(mock_valid_token_json)

    # Configura il mock per Credentials.from_authorized_user_file
    mock_creds_instance = MockCredentials.from_authorized_user_file.return_value
    mock_creds_instance.valid = True # Simula credenziali valide
    ...
    MockInstalledAppFlow.from_client_secrets_file.assert_not_called() # Il flusso non dovrebbe partire
...
def test_load_credentials_expired_token_refresh_success(
    MockInstalledAppFlow: MagicMock,
    MockCredentials: MagicMock,
    MockRequest: MagicMock, # Mock per google.auth.transport.requests.Request
    mock_settings: tuple[Path, Path]
):
    """Testa il refresh di un token scaduto che va a buon fine."""
    mock_token_file, _ = mock_settings

    # Crea un finto token.json scaduto ma con refresh_token
    mock_expired_token_json = '''{"token": "expired_token", "refresh_token": "valid_refresh_token", "scopes": ["test_scope"], "client_id": "id", "client_secret": "secret", "expiry": "2000-01-01T00:00:00Z"}'''
    mock_token_file.write_text(mock_expired_token_json)

    # Configura il mock per Credentials
    mock_creds_instance = MockCredentials.from_authorized_user_file.return_value
    mock_creds_instance.valid = False # Non valido inizialmente
    mock_creds_instance.expired = True
    mock_creds_instance.refresh_token = "valid_refresh_token"
    
    # Simula che il refresh abbia successo e renda le credenziali valide
    def refresh_side_effect(request):
        mock_creds_instance.valid = True
        mock_creds_instance.expired = False
        mock_creds_instance.token = "refreshed_token" # Simula un nuovo token
        # Simula che to_json() restituisca il token aggiornato
        mock_creds_instance.to_json.return_value = '''{"token": "refreshed_token", "refresh_token": "valid_refresh_token"}'''
        return None # Il metodo refresh non restituisce nulla
    ...
    MockInstalledAppFlow.from_client_secrets_file.assert_not_called() # Il flusso non dovrebbe partire
    assert mock_token_file.read_text() == '''{"token": "refreshed_token", "refresh_token": "valid_refresh_token"}''' # Verifica che il token sia stato salvato
    assert credentials == mock_creds_instance
    assert credentials.valid # Ora dovrebbe essere valido

# TODO: Aggiungere test per i casi di fallimento del refresh e 
#       quando client_secret.json non viene trovato (dovrebbe restituire None e loggare errore). 